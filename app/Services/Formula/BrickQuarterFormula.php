...
class BrickQuarterFormula implements FormulaInterface
{
    ...
    public function trace(array $params): array
    {
        ...
        $kolomVertikalBataRaw = $n(($tinggiDinding - $tebalAdukan / 100) / (($lebarBata + $tebalAdukan) / 100));
        ...
        $barisHorizontalAdukanRaw = $n(($tinggiDinding - $tebalAdukan / 100) / (($lebarBata + $tebalAdukan) / 100) + 1);
        ...
        $volumeAdukanPekerjaan = $n($luasAdukan * ($tinggiBata / 100));
        ...
        $trace['steps'][] = [
            ...
            'calculations' => [
                'Perhitungan' => NumberHelper::format($luasAdukan) . " × $tinggiBata / 100",
