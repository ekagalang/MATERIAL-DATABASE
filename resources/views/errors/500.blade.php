<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>500 - Server Error</title>
    <link href="https://fonts.googleapis.com/css2?family=League+Spartan:wght@400;700;900&display=swap" rel="stylesheet">
    <style>
        html, body { height: 100%; width: 100%; margin: 0; overflow: hidden; background: linear-gradient(135deg, #450a0a 0%, #891313 100%); font-family: 'League Spartan', sans-serif; }
        body::before { content: ""; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-image: linear-gradient(rgba(255, 255, 255, 0.08) 1px, transparent 1px), linear-gradient(90deg, rgba(255, 255, 255, 0.08) 1px, transparent 1px); background-size: 50px 50px; pointer-events: none; }
        .error { position: absolute; left: 10%; top: 50%; transform: translateY(-50%); color: #ffffff; z-index: 10; max-width: 500px; }
        .error__title { font-size: 8em; font-weight: 900; line-height: 1; text-shadow: 4px 4px 0px #2a0404; color: #fca5a5; margin: 0; }
        .error__subtitle { font-size: 2.5em; font-weight: 700; margin-bottom: 20px; color: #fbbf24; }
        .error__description { opacity: 0.9; font-size: 1.2em; line-height: 1.5; color: #e2e8f0; margin-bottom: 40px; }
        .error__button { padding: 12px 30px; border: 2px solid #fbbf24; background-color: transparent; border-radius: 8px; color: #fbbf24; cursor: pointer; transition: 0.2s; font-size: 1em; font-weight: 700; text-decoration: none; display: inline-block; margin-right: 15px; }
        .error__button:hover { background-color: rgba(251, 191, 36, 0.1); }
        .error__button--active { background-color: #fbbf24; border: 2px solid #fbbf24; color: #450a0a; }

        .scene { position: absolute; right: 15%; top: 50%; transform: translateY(-50%); width: 400px; height: 500px; }

        /* Falling Debris */
        .debris { position: absolute; width: 40px; height: 20px; background: #7f1d1d; animation: fall 3s infinite linear; opacity: 0; border-radius: 4px; }
        .d1 { left: 50px; animation-delay: 0s; background: #b91c1c; }
        .d2 { right: 80px; animation-delay: 1.5s; width: 30px; height: 30px; border-radius: 50%; background: #991b1b; }
        .d3 { left: 150px; animation-delay: 0.5s; width: 10px; height: 40px; background: #ef4444; }
        
        @keyframes fall {
            0% { top: -50px; opacity: 1; transform: rotate(0deg); }
            80% { opacity: 1; }
            100% { top: 500px; opacity: 0; transform: rotate(360deg); }
        }

        /* Worker Crouching/Fallen */
        .worker { 
            position: absolute; bottom: 0; right: 50px; width: 220px; height: 300px; 
            transform-origin: bottom center;
        }

        /* Torso (Crouched) */
        .torso { 
            position: absolute; bottom: 60px; left: 45px; width: 110px; height: 100px; 
            background: #f97316; border-radius: 25px 25px 10px 10px; z-index: 5; 
            transform: rotate(-10deg);
        }
        .vest-stripe { position: absolute; background: #cbd5e1; height: 100%; width: 20px; top: 0; }
        .vs-1 { left: 20px; } .vs-2 { right: 20px; } .vest-h { position: absolute; bottom: 20px; height: 20px; width: 100%; background: #cbd5e1; }
        .shirt-collar { position: absolute; top: -5px; left: 35px; width: 40px; height: 20px; background: #1e293b; border-radius: 0 0 20px 20px; z-index: 6; }

        /* Legs (Bent) */
        .leg { position: absolute; bottom: 0; width: 45px; height: 80px; background: #1e3a8a; border-radius: 10px; z-index: 4; }
        .leg-left { left: 30px; bottom: 10px; transform: rotate(45deg); }
        .leg-right { right: 60px; bottom: 10px; transform: rotate(-45deg); }

        .boot { position: absolute; width: 50px; height: 25px; background: #3f2c22; border-radius: 10px 10px 0 0; border-bottom: 5px solid #1c1917; z-index: 5; }
        .boot-left { left: 10px; bottom: 0; transform: rotate(10deg); }
        .boot-right { right: 40px; bottom: 0; transform: rotate(-10deg); }

        /* Head (Looking Up/Scared) */
        .head-group { 
            position: absolute; top: 45px; left: 50px; width: 80px; height: 90px; 
            transform-origin: bottom center; z-index: 4; 
            animation: duck 3s infinite ease-in-out;
        }
        .neck { position: absolute; bottom: 0px; left: 25px; width: 30px; height: 25px; background: #f1dca7; border-radius: 10px; }
        .face { position: absolute; top: 0; width: 70px; height: 75px; left: 5px; background: #f1dca7; border-radius: 20px; }
        
        /* Helmet (Askew) */
        .helmet { 
            position: absolute; top: -25px; left: -10px; width: 90px; height: 45px; 
            background: #fbbf24; border-radius: 50px 50px 10px 10px; border-bottom: 5px solid #d97706; 
            transform: rotate(-15deg);
        }

        /* Arms (Shielding Head) */
        .arm { position: absolute; width: 30px; height: 90px; background: #1e293b; border-radius: 15px; z-index: 6; }
        
        .arm-left { 
            top: 145px; left: 30px; 
            transform-origin: 15px 15px; 
            transform: rotate(140deg); 
        }
        .forearm-left {
            position: absolute; bottom: -40px; width: 30px; height: 50px; background: #f1dca7; border-radius: 15px;
            transform-origin: top center; transform: rotate(-90deg);
        }

        .arm-right { 
            top: 130px; right: 60px; 
            transform-origin: 15px 15px; 
            transform: rotate(-140deg); 
        }
        .forearm-right {
            position: absolute; bottom: -40px; width: 30px; height: 50px; background: #f1dca7; border-radius: 15px;
            transform-origin: top center; transform: rotate(90deg);
        }

        @keyframes duck {
            0%, 100% { transform: translateY(0) rotate(0deg); }
            50% { transform: translateY(5px) rotate(5deg); }
        }
    </style>
</head>
<body>
    <div class="error">
        <h1 class="error__title">500</h1>
        <div class="error__subtitle">Kecelakaan Server</div>
        <div class="error__description">Oops! Terjadi keruntuhan struktur pada server kami. Tim teknisi sedang berusaha mengevakuasi data.</div>
        <a href="/" class="error__button error__button--active">REFRESH HALAMAN</a>
        <a href="/" class="error__button">LAPORKAN INSIDEN</a>
    </div>

    <div class="scene">
        <div class="debris d1"></div>
        <div class="debris d2"></div>
        <div class="debris d3"></div>

        <div class="worker">
            <div class="leg leg-left"></div><div class="leg leg-right"></div>
            <div class="boot boot-left"></div><div class="boot boot-right"></div>
            <div class="torso">
                <div class="vest-stripe vs-1"></div><div class="vest-stripe vs-2"></div><div class="vest-h"></div><div class="shirt-collar"></div>
            </div>
            <div class="head-group">
                <div class="neck"></div><div class="face"></div><div class="helmet"></div>
            </div>
            <div class="arm arm-left"><div class="forearm-left"></div></div>
            <div class="arm arm-right"><div class="forearm-right"></div></div>
        </div>
    </div>
</body>
</html>