...
<div class="card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h2>Database cat</h2>
        <a href="{{ route('cats.create') }}" class="btn btn-success open-modal">+ Tambah cat</a>
    </div>

    <!-- Search Form -->
    <form action="{{ route('cats.index') }}" method="GET" style="margin-bottom: 20px;">
        <div style="display: flex; gap: 10px;">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari cat, merek, warna, toko..." style="flex: 1; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="actions" style="justify-content: center;">
                                <a href="{{ route('cats.show', $cat->id) }}" class="btn btn-primary btn-sm open-modal">Detail</a>
                                <a href="{{ route('cats.edit', $cat->id) }}" class="btn btn-warning btn-sm open-modal">Edit</a>
                                <form action="{{ route('cats.destroy', $cat->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus cat ini?')">
                                    @csrf
                                    @method('DELETE')
    ...
    document.querySelectorAll('.open-modal').forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            .then(html => {
                ...
                // Load cat-form.js if not loaded
