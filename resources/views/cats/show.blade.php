    ...
    <div style="display: flex; gap: 40px; margin-top: 20px;">
        ...
        <div style="flex: 1;">
            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    <td style="padding: 12px; font-weight: 600; width: 180px; border-bottom: 1px solid #e3e3e0; background: #f8f9fa;">Nama Cat</td>
                    <td style="padding: 12px; border-bottom: 1px solid #e3e3e0;"><strong>{{ $cat->cat_name }}</strong></td>
                </tr>
                <tr>
                    <td style="padding: 12px; font-weight: 600; border-bottom: 1px solid #e3e3e0; background: #f8f9fa;">Jenis</td>
                    <td style="padding: 12px; border-bottom: 1px solid #e3e3e0;">{{ $cat->type ?? '-' }}</td>
                </tr>
                <tr>
                    <td style="padding: 12px; font-weight: 600; border-bottom: 1px solid #e3e3e0; background: #f8f9fa;">Merek</td>
                    <td style="padding: 12px; border-bottom: 1px solid #e3e3e0;">{{ $cat->brand ?? '-' }}</td>
                </tr>
                <tr>
                    <td style="padding: 12px; font-weight: 600; border-bottom: 1px solid #e3e3e0; background: #f8f9fa;">Sub Merek</td>
                    <td style="padding: 12px; border-bottom: 1px solid #e3e3e0;">{{ $cat->sub_brand ?? '-' }}</td>
                </tr>
                <tr>
                    ...
                    <td style="padding: 12px; border-bottom: 1px solid #e3e3e0;">
                        @if($cat->color_name || $cat->color_code)
                            {{ $cat->color_name ?? '-' }}
                            @if($cat->color_code)
                                <span style="color: #7f8c8d; font-size: 12px;">({{ $cat->color_code }})</span>
                            @endif
                        @else
                            -
                        @endif
                ...
                <tr>
                    <td style="padding: 12px; font-weight: 600; border-bottom: 1px solid #e3e3e0; background: #f8f9fa;">Volume</td>
                    <td style="padding: 12px; border-bottom: 1px solid #e3e3e0;">
                        @if($cat->volume)
                    ...
                    <td style="padding: 12px; border-bottom: 1px solid #e3e3e0;">
                        @if($cat->package_unit)
                            ...
                            @if($cat->package_weight_gross)
                                - {{ number_format($cat->package_weight_gross, 2, ',', '.') }} Kg (Kotor)
                            @endif
                        @else
                            -
                        @endif
                ...
                <tr>
                    <td style="padding: 12px; font-weight: 600; border-bottom: 1px solid #e3e3e0; background: #f8f9fa;">Harga Beli</td>
                    <td style="padding: 12px; border-bottom: 1px solid #e3e3e0;">
                        @if($cat->purchase_price)
                            <strong style="color: #e67e22;">Rp {{ number_format($cat->purchase_price, 0, ',', '.') }}</strong> / {{ $cat->price_unit ?? 'unit' }}
                        @else
                            -
                        @endif
                    </td>
                ...
                <tr>
                    ...
                    <td style="padding: 12px; border-bottom: 1px solid #e3e3e0;">
                        @if($cat->comparison_price_per_kg)
                            <strong style="color: #27ae60;">Rp {{ number_format($cat->comparison_price_per_kg, 0, ',', '.') }}</strong> / Kg
                        @else
                            -
                        @endif
                    </td>
